...
export default function ManageBanks() {
  const bnks = useSelector((state) => state.banksList.data);
  const [banks, setbanks] = useState(bnks);
  ...
  // useEffect(() => {
  //   !user.access.manageBanks && history.push("/noaccess");
  // }, []);

  useEffect(() => {
    if (searchVal) {
      var text = _.toLower(searchVal);
      var res = _.filter(bnks, function (object) {
        return _(object)
          .omit(["NOTE_2", "NOTE_1"])
  ...
  useEffect(() => {
    setlength(banks.filter((item) => item.STATUS).length);
  }, [banks]);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = banks.filter((item) => item.STATUS);
      ...
      <Paper className={classes.paper}>
        ...
        <TableContainer className={classes.container}>
          ...
          >
            ...
            />
            <TableBody>
              {stableSort(banks, getComparator(order, orderBy)).map(
                (row, index) => {
      ...
      <TableRow>
        <TableCell padding="checkbox">
          <Checkbox
            ...
            inputProps={{ "aria-label": "select all banks" }}
          />
        </TableCell>
...
const EnhancedTableToolbar = ({
  ...
  const { deleteCustomer, getBanksList } = useIt();
  const loading = useSelector((state) => state.banksList.deleteLoading);
      ...
      {numSelected > 0 ? (
        ...
        <Fragment>
          ...
          <Tooltip title="Refresh">
            <IconButton aria-label="Refresh" onClick={getBanksList}>
              <RefreshRoundedIcon />
            </IconButton>
          </Tooltip>
          <Menu
