...
export default function ManageCustomers() {
  const custs = useSelector((state) => state.customersList.data);
  ...
  // useEffect(() => {
  //   !user.access.manageCustomers && history.push("/noaccess");
  // }, []);
  ...
  useEffect(() => {
    setlength(customers.filter((item) => item.STATUS).length);
  }, [customers]);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = customers.filter((item) => item.STATUS);
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">
                        <a
                          href={GOOGLE_IMAGE + row.IMAGE}
                          target="_blank"
                          rel="noreferrer"
                        >
                          <Avatar src={GOOGLE_IMAGE + row.IMAGE} />
                        </a>
                      </TableCell>
...
const EnhancedTableToolbar = ({
  ...
  const { deleteCustomer, getCustomersList } = useIt();
  const loading = useSelector((state) => state.customersList.deleteLoading);
      ...
      {numSelected > 0 ? (
        ...
        <Fragment>
          ...
          <Tooltip title="Refresh">
            <IconButton aria-label="Refresh" onClick={getCustomersList}>
              <RefreshRoundedIcon />
            </IconButton>
          </Tooltip>
          <Menu
